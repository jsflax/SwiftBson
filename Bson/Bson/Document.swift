...
public struct Document: Collection, BsonCodable, Bson {
    ...
    private func readValue(reader: BsonReader, decoderContext: DecoderContext) throws -> Any? {
        ...
        if let transformer = decoderContext.transformer {
            return transformer.transform(
                objectToTransform: try defaultBsonTypeDecoderClassMap[bsonType]!(reader, decoderContext))
        } else {
            return try defaultBsonTypeDecoderClassMap[bsonType]!(reader, decoderContext)
        }
    }
